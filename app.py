...
prob_min, prob_max = float(df[COL_PROB].min(skipna=True)), float(df[COL_PROB].max(skipna=True))
...
st.sidebar.caption(f"{prob_range[0]*100:.0f}% – {prob_range[1]*100:.0f}%")
filtered_df = filtered_df[
    (filtered_df[COL_PROB] >= prob_range[0]) & (filtered_df[COL_PROB] <= prob_range[1])
]
