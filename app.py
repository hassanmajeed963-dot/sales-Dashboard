...
used_cols = {c for c in [region_col, rep_col, date_col] if c}
...
for c in [revenue_col, units_col]:
    if c:
        df[c] = pd.to_numeric(
...
if revenue_col:
    ...
    avg_rev = filtered_df[revenue_col].mean(skipna=True)
    kpi2.metric("Total Revenue", f"{total_rev:,.0f}" if pd.notna(total_rev) else "0")
    kpi3.metric("Avg Revenue / Row", f"{avg_rev:,.0f}" if pd.notna(avg_rev) else "0")
if units_col:
    total_units = filtered_df[units_col].sum(skipna=True)
    kpi4.metric("Total Units", f"{total_units:,.0f}" if pd.notna(total_units) else "0")
